...
对于具有大量可视化单词的码本，这种聚合表示的存储成本可能令人望而却步。

二值化是在这些情况下允许可伸缩检索的有效策略。我们采用了与[38]类似的二值化策略，如果x>0，可以通过元素函数b(x)=+1得到ΦR，否则−1。我们用a\*上标表示二值化版本(例如，R-ASMK*是R-ASMK的二值化版本。

# **5. Experiments**

我们提出了两种类型的实验：

首先，地标检测，以评估在新数据集上训练的目标探测器模型的质量。

其次，我们利用检测到的地标来增强图像检索系统。

## **5.1. Landmark Detection**

我们训练两种检测模型 在第3章上我们已经收集和描述的边界盒数据。 一个单镜头基于Mobilenet-V2[33]的SSD探测器[20]和一个基于两级Resnet-50[10]的Faster-RCNN[32]。评估时使用标准目标检测评估度量50%的平均精度(AP)。

在500k步内，两种模型在验证集上均达到85%左右（分别为85.61%和84.37%）。

这些模型使用公开可用的Tensorflflow 对象检测API[11]进行训练。

结果表明，利用我们的数据集可以训练精确的地标定位。 Mobilenet-V2-SSD的运行速度每张图像运行27 ms，而Resnet-50-Faster-RCNN运行89 ms，这两个数字都在TitanXGPU上。

## **5.2. Image Retrieval**

我们进行了区域搜索和区域聚合实验。下面描述了实验设置。

**Datasets**

我们使用Oxford[26]和Paris[27]数据集，它们最近被重新访问以纠正标注错误，添加新的查询图像并引入新的评估协议[28]；这些数据集分别被称为ROxf和RPar。

每个数据集有70个查询图像，在ROxf(RPar)数据集中有4993个（6322）个数据库图像。

我们报告中等和困难设置的结果；

对于消融，我们更具体地关注困难设置。使用平均平均精度(mAP)和第10级（mP@10）的平均精度测量性能。

我们还使用R1M干扰物集[28]进行了大规模的实验，其中包含1,001,001张图像

**Image representation.**

除另有说明外，我们在实验中使用以下设置。

使用发布的DELF模型[25]（在[9]的数据集上进行预训练），默认配置（每个区域最多提取1000个特征，所需的最小注意力分数为100），特征维数设置为128

在计算聚合内核时使用1024大小的codebooks；作为常见的做法，codebooks在ROxf上进行训练，以便在RPar上进行检索实验，反之亦然。

我们专注于改进用于检索的核心图像表示，而不考虑 query expansion(QE)[7]技术，如s Hamming QE[39]、αQE[30]或diffusion[13,12]；这些方法可以纳入我们的系统，以获得更强的检索性能。



**Region selection techniques.**

对于我们的Detect-to-Retrieve(D2R)框架，我们采用了Sec5.1.中描述的训练好的更快的R-CNN检测器。

我们与之前提出的图像检索的区域选择技术进行了比较：uniform grid 和 Selective Search (SS)

为了改变每张图像的区域数量，我们执行以下操作：

(i)对于D2R，我们改变地标检测器阈值；

(ii)对于RMACB，我们扫描the number of levels从1到3；

(iii)对于SS，我们选择每张图像的顶部{1、2、5、10个}盒子（在本例中，没有与区域相关的置信分数）。对于所有的区域选择技术，我们将原始图像添加为所选择的区域之一。

**Implementation details.** 

我们在Python/Tensorflow中从头开始实现了聚合的内核框架。

作为与参考MATLAB实现[38]的比较，我们的ASMK*使用1024大小的codebook和 在ROxf-Hard数据集中获得37.91%的mAP的DELF特征，而参考实现获得37.08%。

请注意，参考实现使用了与Hamming Embedding (HE)[14]类似的配置，在二值化之前有一个投影矩阵，相对于中值计算的残差和IDF。我们没有发现使用这些方法有一致的改进，所以我们使用了在章节 4.中描述的更简单的版本。

类似地，参考实现使用多个视觉单词分配，但我们的初步实验显示，使用单一分配改进了结果，使检索更快、更简单。因此我们在实验中采用单一分配。我们扩展了这个实现，以支持我们的区域搜索和聚合技术。
